  ...
  return (
    <div>
      <div className='flex justify-center my-10 mx-2'>
        <div className='flex flex-col justify-evenly bg-white rounded-3xl px-12'>
          {uniFAQ.map((n, i) => (
            <div className='w-full border-b border-gray-300 cursor-pointer my-4  mx-6' onClick={() => handleOpen(i)} key={i} >
                ...
                <h4 className='font-semibold md:text-[23px] text-[14px] transition ease-in hover:opacity-60 '>{n.title}</h4>
                <div className={`pr-8`}>
              ...
              <div className={`grid overflow-hidden transition-all duration-300 ease-in-out  ${open === i ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                  <p className={`overflow-hidden mb-4 font-sans mt-1 max-w-[930px] transition ease-in-out duration-1000 `}>{n.para}</p>
              </div>
            </div>
          ))}
        </div>
